...
test_that("fit_constrained: equalities", {
  ...
  theta = c(
    alpha = 0.8, delta = -0.8, eta = exp(1.2607766403115003),
    phi = exp(1.9912508051913044), nu = exp(0.29620498943510502)
  )

  rss_value <- 0.098711571468416318

  fitted_values <- rep(
    c(
      0.8, 0.77882319854207154, 0.6767231859003154, 0.4987316702071780,
      0.3195612767442848, 0.192615181678511, 0.000079037622388, 2.3427375e-08
    ),
    k
  )

  residuals <- c(
    0.0527, -0.0429, 0.1370, 0.04867680145792846, -0.00102319854207154,
    0.10577680145792846, -0.1206231859003154, 0.0280768140996846,
    0.0465683297928220, -0.0127316702071780, 0.0363683297928220,
    -0.0588316702071780, 0.0357387232557152, -0.0063612767442848,
    0.0303387232557152, -0.050515181678511, 0.016720962377612,
    0.138520962377612, 0.122699976572625
  )
...
test_that("nauc: decreasing", {
  ...
  expect_equal(nauc(result), 0.57634025113166049)
  expect_equal(nauc(result, xlim = c(0, 2)), 0.87325260271259105)
  expect_equal(nauc(result, ylim = c(0.3, 0.7)), 0.61191350930729916)
...
test_that("naac: decreasing", {
  ...
  expect_equal(naac(result), 1 - 0.57634025113166049)
  expect_equal(naac(result, xlim = c(0, 2)), 1 - 0.87325260271259105)
  expect_equal(naac(result, ylim = c(0.3, 0.7)), 1 - 0.61191350930729916)
...
test_that("nauc: increasing", {
  ...
  expect_equal(nauc(result), 0.49315307829602008)
  expect_equal(nauc(result, xlim = c(0, 2)), 0.16335338880480354)
  expect_equal(nauc(result, ylim = c(0.3, 0.7)), 0.49785365693991776)
...
test_that("naac: increasing", {
  ...
  expect_equal(naac(result), 1 - 0.49315307829602008)
  expect_equal(naac(result, xlim = c(0, 2)), 1 - 0.16335338880480354)
  expect_equal(naac(result, ylim = c(0.3, 0.7)), 1 - 0.49785365693991776)
