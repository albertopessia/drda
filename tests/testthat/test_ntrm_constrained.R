...
test_that("ntrm_constrained: Rosenbrock function", {
  ...
  result <- ntrm_constrained(
    fn, gradient, hessian, c(2.5, -0.8), max_iter, c(0, -1), c(3,  5)
  )
  expect_equal(result$optimum, c(1, 1))

  result <- ntrm_constrained(
    fn, gradient, hessian, c(2.5, -0.8), max_iter, c(-Inf, -3), c(3,  Inf)
  )
  expect_equal(result$optimum, c(1, 1))

  # initial point is outside the feasible region
  result <- ntrm_constrained(
    fn, gradient, hessian, c(-5, 10), max_iter, c(0, -1), c(3,  5)
  )
  expect_equal(result$optimum, c(1, 1))

  result <- ntrm_constrained(
    fn, gradient, hessian, c(10, -10), max_iter, c(-Inf, -3), c(3,  Inf)
  )
  expect_equal(result$optimum, c(1, 1))
  ...
  for (i in 1:5) {
    result <- ntrm_constrained(
      fn, gradient, hessian, rnorm(2, sd = 3), max_iter, c(0, -1), c(3,  5)
    )
    expect_equal(result$optimum, c(1, 1))

    result <- ntrm_constrained(
      fn, gradient, hessian, rnorm(2, sd = 3), max_iter, c(-Inf, -3), c(3,  Inf)
    )
    expect_equal(result$optimum, c(1, 1))
  }
  ...
  # initial point is within the feasible region
  result <- ntrm_constrained(
    fn, gradient, hessian, c(0, 0), max_iter, c(-0.5, -0.5), c(0.5, 0.5)
  )
