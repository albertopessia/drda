context("Unconstrained Newton trust region - auxiliary functions")
...
test_that("ntrm_unconstrained: Rosenbrock function", {
  set.seed(2823440)
  ...
  for (i in 1:5) {
    result <- ntrm_unconstrained(fn, gradient, hessian, rnorm(2, sd = 3), 1000)
    expect_equal(result$optimum, c(1, 1))
  }
})

context("Unconstrained Newton trust region method")
...
test_that("ntrm_unconstrained: Rosenbrock function", {
  ...
  # try different random starting points
  for (i in 1:5) {
    result <- ntrm_unconstrained(fn, gradient, hessian, rnorm(2), 1000)
    expect_equal(result$optimum, c(1, 1))
  }
})
