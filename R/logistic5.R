...
#'
#' @export
logistic5_function <- function(x, theta) {
  alpha <- theta[1]
  beta <- theta[2]
  eta <- theta[3]
  phi <- theta[4]
...
#'   length(x)-by-5, where each row is the gradient of the logistic function at
#'   the corresponding element of `x`. Element `H` is an array of dimension
#'   length(x)-by-5-by-5, where `H[k, , ]` is the 5-by-5 Hessian matrix at
#'   `x[k]`.
...
logistic5_rss <- function(stats) {
  function(theta) {
    theta[2] <- theta[1] + exp(theta[2])
    theta[5] <- exp(theta[5])

    mu <- logistic5_function(stats[, 1], theta)
    sum(stats[, 2] * (stats[, 3] - mu)^2)
  }
}

#' @rdname logistic5_rss
logistic5_rss_fixed <- function(stats, known_param) {
  function(z) {
...
logistic5_rss_gradient_hessian <- function(stats) {
  function(theta) {
    theta[2] <- theta[1] + exp(theta[2])
    theta[5] <- exp(theta[5])

    mu <- logistic5_function(stats[, 1], theta)
    mu_gradient_hessian <- logistic5_gradient_hessian(stats[, 1], theta)
    ...
    hessian <- array(0, dim = c(nrow(stats), 5, 5))
...
#' @rdname logistic5_rss_gradient_hessian
logistic5_rss_gradient_hessian_fixed <- function(stats, known_param) {
  function(z) {
    idx <- is.na(known_param)
  ...
  for (eta in eta_set) {
    for (phi in phi_set) {
      for (log_nu in log_nu_set) {
        ...
        g <- 1 / (1 + nu * exp(-eta * (stats[, 1] - phi)))^(1 / nu)
        ...
        current_par <- c(alpha, log_beta, eta, phi, log_nu)
...
logistic5_fisher_info_normal <- function(stats, n, theta, sigma) {
  alpha <- theta[1]
  beta <- theta[2]
  eta <- theta[3]
  phi <- theta[4]
  ...
  gradient <- matrix(1, nrow = nrow(stats), ncol = 5)
  ...
  tmp <- array(0, dim = c(nrow(stats), 5, 5))
...
#' Fit a 5-parameter logistic function
#'
#' Use a Newton trust-region method to fit a 5-parameter logistic function to
#' observed data.
#'
#' @param stats numeric matrix of sufficient statistics.
...
#' @param lower_bound numeric vector of length 5 with the minimum admissible
#'   values.
#' @param upper_bound numeric vector of length 5 with the maximum admissible
#'   values.
...
#'   }
logistic5_ntrm <- function(stats, start, max_iter) {
  init <- if (!is.null(start)) {
    start
  } else {
    logistic5_init(stats)
  }

  fn <- logistic5_rss(stats)
  gh <- logistic5_rss_gradient_hessian(stats)

  ntrm(fn, gh, init, max_iter)
}

#' @rdname logistic5_ntrm
logistic5_ntrm_constrained <- function(
  ...
  init <- if (!is.null(start)) {
    ...
    ifelse(is.na(known_param), logistic5_init(stats), known_param)
  }
  ...
  if (any(constraint[, 2])) {
    ...
    fn <- logistic5_rss_fixed(stats, known_param)
...
#' @param max_iter maximum number of iterations in the optimization algorithm.
#' @param lower_bound numeric vector of length 5 with the minimum admissible
#'   value of `alpha`, `omega`, `eta`, `phi`, and `nu'` respectively. Values can
#'   be equal to `-Inf`.
#' @param upper_bound numeric vector of length 5 with the maximum admissible
#'   value of `alpha`, `omega`, `eta`, `phi`, and `nu'` respectively. Values
#'   can be equal to `Inf`.
#'
#' @return A list with the following components:
#'   \describe{
...
#'   }
logistic5_fit <- function(x, y, start, max_iter) {
  stats <- suff_stats(x, y)

  solution <- logistic5_ntrm(stats, start, max_iter)
  ...
  result <- list(
    ...
    df.residual = length(y) - 5,
    fitted.values = logistic5_function(x, theta)
  )
...
#' @rdname logistic5_fit
logistic5_fit_constrained <- function(
  ...
  solution <- logistic5_ntrm_constrained(
    stats, start, max_iter, constraint, lower_bound, upper_bound, known_param
  )
...
#' @rdname logistic5_fit
logistic5_fit_weighted <- function(x, y, w, start, max_iter) {
  w_zero <- w == 0
  w_positive <- !w_zero
    ...
    if (length(y) == 0) {
      ...
      return(
        list(
          ...
          iterations = 0L,
          constrained = FALSE,
          estimated = rep(NA_real_, 5),
          coefficients = rep(NA_real_, 5),
          rss = NA_real_,
          df.residual = 0L,
...
#' @rdname logistic5_fit
logistic5_fit_weighted_constrained <- function(
